...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Admin: seletor de clientes */}
      {isAdmin && (
        <div className="px-4 py-4 border-b border-[#1a1a1a]">
          <p className="text-label mb-3">Cliente ativo</p>
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${
                  activeClient?.id === c.id
                    ...
                    : 'text-[#888888] hover:bg-[#1a1a1a] hover:text-slate-200'
                }`}
              >
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: c.color }} />
                <span className="truncate flex-1 text-left">{c.name}</span>
                {activeClient?.id === c.id && <ChevronRight size={12} />}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Cliente: exibe apenas o nome da empresa */}
      {!isAdmin && activeClient && (
        <div className="px-4 py-4 border-b border-[#1a1a1a]">
          <p className="text-label mb-2">Empresa</p>
          <div className="flex items-center gap-2 px-3 py-2 bg-orange-500/10 rounded-lg border border-orange-500/20">
            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: activeClient.color }} />
            <span className="text-sm text-orange-400 font-medium truncate">{activeClient.name}</span>
          </div>
        </div>
      )}
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                isActive
                  ? 'bg-orange-500 text-white shadow-lg shadow-orange-950/60'
                  : 'text-[#888888] hover:bg-[#1a1a1a] hover:text-slate-200'
              }`
            }
          >
      ...
      <div className="px-4 py-4 border-t border-[#1f1f1f] space-y-1">
        <NavLink
          ...
          className={({ isActive }) =>
            `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
              isActive ? 'bg-orange-500 text-white' : 'text-[#888888] hover:bg-[#1a1a1a] hover:text-slate-200'
            }`
          }
        >
          <Settings size={18} />
        ...
        <button
          onClick={signOut}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-[#888888] hover:bg-red-500/10 hover:text-red-400 transition-all"
        >
          <LogOut size={18} />
          Sair
        </button>
