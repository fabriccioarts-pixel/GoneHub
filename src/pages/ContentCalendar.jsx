        ...
        <div className="bg-[#1a1a1a]/50 border border-[#2a2a2a]/50 rounded-xl p-5">
          ...
          <div className="space-y-2">
            {filtered.sort((a, b) => a.day - b.day).map(p => {
              const { icon: Icon, color } = TYPES[p.type] || TYPES.Feed
              return (
                <div key={p.id} className="flex items-center gap-3 py-2.5 px-3 bg-[#222]/30 rounded-lg hover:bg-[#222]/50 transition-all">
                  <span className="text-xs text-[#555] w-6 text-center">{p.day}</span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {p.caption && <p className="text-xs text-[#555] truncate">{p.caption}</p>}
                  </div>
                  ...
                  {isAdmin && (
                    <>
                      ...
                      <button onClick={() => removePost(p.id)} className="text-[#444] hover:text-red-400 transition-colors"><X size={14} /></button>
                    </>
                  )}
